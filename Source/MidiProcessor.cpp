...
void MidiProcessor::stopCurrentNotes(const int velocity, const int samplePosition)
{
    if (arpeggiatorParams.activated->get()) { return; }

    for (const auto& noteState: currentOutputNotesOn) {
        processedMidi.addEvent(juce::MidiMessage::noteOff(noteState.channel, noteState.note), samplePosition);
    }
}
